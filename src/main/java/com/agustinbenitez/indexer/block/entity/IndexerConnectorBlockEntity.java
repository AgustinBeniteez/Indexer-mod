...
public class IndexerConnectorBlockEntity extends RandomizableContainerBlockEntity {
    ...
    private net.minecraft.core.NonNullList<ItemStack> items = net.minecraft.core.NonNullList.withSize(BASE_FILTER_SLOTS,
            ItemStack.EMPTY);
        ...
        if (tag.contains("FilterItems")) {
            ...
            for (int i = 0; i < maxRead; i++) {
                if (filterItemsTag.contains("Item" + i)) {
                    this.filterItems.add(ItemStack.parseOptional(registries, filterItemsTag.getCompound("Item" + i)));
                } else {
    ...
    public static void tick(Level level, BlockPos pos, BlockState state, IndexerConnectorBlockEntity entity) {
        ...
        if (entity.tickCounter >= 20) {
            ...
            if (state.getValue(IndexerConnectorBlock.CONNECTED) != isConnected) {
                level.setBlock(pos, state.setValue(IndexerConnectorBlock.CONNECTED, isConnected),
        ...
        // Ya no enviamos mensajes de notificación al chat cuando se conecta un
        // contenedor
    }
            ...
            if (adjacentEntity instanceof Container && !(adjacentEntity instanceof IndexerConnectorBlockEntity)) {
                ...
                if (isChestBlockEntity(adjacentEntity)) {
                    ...
                    if (doubleChestPos != null) {
                        // Es un cofre doble, usar la posición del cofre "principal" (el de menor
                        // coordenada)
        ...
        for (Direction direction : horizontalDirections) {
            ...
            if (adjacentEntity != null && isChestBlockEntity(adjacentEntity)) {
                ...
                if (chestState.getBlock().getClass().equals(adjacentState.getBlock().getClass())) {
                    ...
                    try {
                        if (chestState.hasProperty(
                                net.minecraft.world.level.block.state.properties.BlockStateProperties.HORIZONTAL_FACING)
                                &&
                                adjacentState.hasProperty(
                                        net.minecraft.world.level.block.state.properties.BlockStateProperties.HORIZONTAL_FACING)) {

                            Direction chestFacing = chestState.getValue(
                                    net.minecraft.world.level.block.state.properties.BlockStateProperties.HORIZONTAL_FACING);
                            Direction adjacentFacing = adjacentState.getValue(
                                    net.minecraft.world.level.block.state.properties.BlockStateProperties.HORIZONTAL_FACING);
    ...
    private boolean isContainerAlreadyConnected(BlockPos containerPos) {
        if (this.level == null || containerPos == null)
            return false;
    ...
    public boolean canAcceptItem(ItemStack stack) {
        ...
        // Si es combustible válido (carbón, carbón vegetal o cubo de lava) y el
        // contenedor es un horno, permitir siempre
        if ((isCoalOrCharcoal || isLavaBucket) && isFurnace) {
            return true;
        }

        // PRIORIDAD ABSOLUTA: Verificar si el item está bloqueado por un filtro de
        // bloqueo
        ...
        // Si solo hay filtros de bloqueo (sin filtros positivos), acepta todo lo que no
        // esté bloqueado
        ...
        // 1. Filtros de nombre (segunda prioridad más alta) - OR lógico entre múltiples
        // nombres
        ...
        for (ItemStack filterItem : this.filterItems) {
            if (!filterItem.isEmpty() && filterItem.getItem() == ModItems.NAME_FILTER.get()) {
                hasNameFilters = true;
                System.out.println("[DEBUG] Evaluando filtro de nombre: " + filterItem.getComponents());
        ...
        // 2. Filtros de atributos/encantamientos (tercera prioridad) - OR lógico entre
        // múltiples
        ...
        // 3. Filtros específicos (herramientas, comida, combustible, mod) - OR lógico
        // entre múltiples
        ...
        for (ItemStack filterItem : this.filterItems) {
            if (!filterItem.isEmpty() &&
                ...
                System.out.println(
                        "[CONNECTOR_DEBUG] Evaluando filtro específico: " + filterItem.getItem().getDescriptionId()
                                + " para item: " + stack.getItem().getDescriptionId());
                if (FilterUtils.passesFilter(stack, filterItem)) {
        ...
        // 4. Filtros exactos (menor prioridad) - OR lógico entre múltiples ítems
        // exactos
    ...
    /**
     * Verifica si un item está bloqueado por algún filtro de bloqueo en este
     * conector
     * 
    ...
    public boolean isItemBlocked(ItemStack stack) {
        for (ItemStack filterItem : this.filterItems) {
            if (!filterItem.isEmpty() && FilterUtils.isBlockingFilter(filterItem)) {
                // Para filtros de bloqueo: si passesFilter devuelve false, significa que el
                // item está bloqueado
                if (!FilterUtils.passesFilter(stack, filterItem)) {
                    return true;
                }
    ...
    public boolean canAcceptBuckets() {
        ...
        BlockPos pos = this.connectedContainerPos;
        if (pos == null) {
            return false;
        }
        ...
        BlockEntity containerEntity = this.level.getBlockEntity(pos);
        if (!(containerEntity instanceof Container) || containerEntity instanceof IndexerConnectorBlockEntity) {
            this.connectedContainerPos = null;
            return false;
        }
    ...
    public ItemStack insertItem(ItemStack stack) {
        ...
        // Si es un horno (AbstractFurnaceBlockEntity) y el ítem es combustible válido
        // (carbón, carbón vegetal o cubo de lava)
        if (isFurnace && (isCoalOrCharcoal || isLavaBucket)) {
            com.agustinbenitez.indexer.IndexerMod.LOGGER.info(
                    "Detected furnace and fuel item (coal/charcoal/lava bucket), attempting to insert into fuel slot");
            ...
            if (FURNACE_FUEL_SLOT < container.getContainerSize()) {
                ...
                // Si llegamos aquí, significa que no pudimos insertar todo el combustible en
                // este horno
            ...
            // Si es un horno y combustible válido, SOLO intentamos insertar en el slot de
            // combustible
        ...
        // Si es un horno pero NO es combustible válido, solo permitir inserción en el
        // slot superior (ingredientes)
        if (isFurnace && !(isCoalOrCharcoal || isLavaBucket)) {
            com.agustinbenitez.indexer.IndexerMod.LOGGER
                    .info("Detected furnace and non-fuel item, attempting to insert into input slot");
        ...
        // Esto soluciona problemas con cofres dobles (detectando el inventario
        // completo) y contenedores de mods
        ...
        /*
         * Bloque legacy eliminado: La lógica manual de cofre doble causaba problemas de
         * posicionamiento
         */
    ...
    // Método auxiliar para insertar items en un cofre doble como inventario
    // unificado
    ...
    private ItemStack insertItemSingle(ItemStack stack, Container container, BlockEntity containerEntity) {
        ...
        if (isChestBlockEntity(containerEntity)) {
            ...
            if (partnerPos != null) {
                ...
                BlockPos secondChestPos = mainChestPos.equals(this.connectedContainerPos) ? partnerPos
                        : this.connectedContainerPos;
    ...
    public ItemStack removeItem(int slot, int amount) {
        if (slot >= 0 && slot < getCurrentFilterSlots() && slot < this.filterItems.size()
                && !this.filterItems.get(slot).isEmpty()) {
        ...
        if (slot >= 0 && slot < getCurrentFilterSlots()) {
            ...
            if (!stack.isEmpty()) {
                ...
                if (!this.filterItems.get(slot).isEmpty()) {
                    ...
                    // Buscar el primer slot disponible más cercano al inicio solo si el slot está
                    // vacío
     ...
     * 
     * @param stack       el ItemStack a verificar
     * @param excludeSlot el slot a excluir de la verificación (-1 para no excluir
     *                    ninguno)
     * @return true si el item ya existe en el filtro, false en caso contrario
     */
     ...
     * 
     * @return el índice del slot vacío más cercano al inicio, o -1 si no hay slots
     *         vacíos
     */
    ...
    protected void setItems(net.minecraft.core.NonNullList<ItemStack> items) {
        this.items = items;
        // Los filtros se manejan por separado, no necesitamos actualizar filterItems
        // aquí
    }
    ...
    public void ensureFilterCapacity() {
        ...
        if (this.items.size() != desired) {
            net.minecraft.core.NonNullList<ItemStack> newItems = net.minecraft.core.NonNullList.withSize(desired,
                    ItemStack.EMPTY);
